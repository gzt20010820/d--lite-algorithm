 ...
 */
void Dstar::setRHS(state u, double rhs) {//double改为void
...
double Dstar::eightCondist(state a, state b) {
  ...
  return ((sqrt(2)-1.0)*min + max);//M_SQRT2改为sqrt(2)
}
...
double Dstar::cost(state a, state b) {
  ...
  if (xd+yd>1) scale = sqrt(2);//M_SQRT2改为sqrt(2)
